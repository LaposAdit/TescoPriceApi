...
import { NapojeResponseDto, NapojeTransformedProductDto, PromotionDto } from 'src/dto/NapojeDTO';

@Injectable()
export class NapojeService {
    ...
    private async fetchProductsFromApi(): Promise<NapojeTransformedProductDto[]> {

        const url = 'https://potravinydomov.itesco.sk/groceries/sk-SK/resources';
        const headers = { ...defaultHeaders }

        let allProducts: NapojeTransformedProductDto[] = [];
        let count = 48;
        let page = 1;
        let morePages = true;
        ...
        while (morePages) {
            const body = {
                ...
                sharedParams: {
                    superdepartment: 'napoje',
                    query: { count: count.toString(), page: page.toString() },
                    referer: `/groceries/sk-SK/shop/napoje/all?count=${count}&page=${page - 1}`
                },
    ...
    private transformData(data: any): NapojeTransformedProductDto[] {
    ...
    private async saveProductsToDb(products: NapojeTransformedProductDto[]) {
        for (const product of products) {
            try {
                // Create a new product record with a unique lastUpdated value
                await this.prisma.napoje.create({
                    data: {
    ...
    async getProducts(update: boolean, page: number, pageSize: number, sale?: boolean): Promise<NapojeResponseDto> {
        if (update) {
            const productsFromApi = await this.fetchProductsFromApi();
            await this.saveProductsToDb(productsFromApi);
        }

        const whereClause: any = { category: 'napoje' };
    ...
    async getProductById(productId: string): Promise<NapojeTransformedProductDto[]> {
        const productsFromDb = await this.prisma.napoje.findMany({
            where: { productId },
            include: { promotions: true },
            orderBy: { lastUpdated: 'desc' }
        });
